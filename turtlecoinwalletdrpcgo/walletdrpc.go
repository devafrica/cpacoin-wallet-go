...
var (
	rpcURL = "http://127.0.0.1:13282/json_rpc"
)

// RequestBalance provides the available and locked balances of the current wallet
// returned balances are expressed in CPA, not in 0.01 CPA
...
// RequestStatus requests cpawalled-service connection and sync status
...
// parameters amount and fee are expressed in CPA, not 0.01 CPA
func SendTransaction(addressRecipient string, amount float64, paymentID string, fee float64, rpcPassword string) (transactionHash string, err error) {
	...
	amountInt := uint64(amount * 1000000) // expressed in hundredth of CPA
	feeInt := uint64(fee * 1000000)       // expressed in hundredth of CPA
...
// threshold is the value that determines which outputs will be optimized. Only the outputs, lesser than the threshold value, will be included into a fusion transaction (threshold is expressed in CPA, not 0.01 CPA).
...
func EstimateFusion(threshold int, addresses []string, rpcPassword string) (fusionReadyCount int, totalOutputCount int, err error) {
	...
	threshold *= 1000000 // expressed in hundredth of CPA
...
// returned fee is expressed in CPA, not in 0.01 CPA
