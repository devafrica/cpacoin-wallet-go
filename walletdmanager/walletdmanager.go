...
func RequestAvailableBalanceToBeSpent(transferFeeString string) (availableBalance float64, err error) {
	...
	transferFee, err := strconv.ParseFloat(transferFeeString, 64) // transferFee is expressed in CPA
	if err != nil {
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string) (transactionHash string, err error) {
	...
	if !strings.HasPrefix(transferAddress, "cp") || (len(transferAddress) != 97 && len(transferAddress) != 187) {
	...
	transferAmount, err := strconv.ParseFloat(transferAmountString, 64) // transferAmount is expressed in CPA
	if err != nil {
		return "", errors.New("amount is invalid")
	}

	if transferAmount <= 0 {
		return "", errors.New("amount of CPA to be sent should be greater than 0")
	}
...
// SaveWallet saves the sync status of the wallet. To be done regularly so when cpawalled-service crashes, sync is not lost
...
// StartWalletd starts the cpawalled-service daemon with the set wallet info
...
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, useCheckpoints bool, daemonAddress string, daemonPort string) (err error) {
	...
	if isWalletdRunning() {
		errorMessage := "cpawalled-service is already running in the background.\nPlease close it via "
	...
	if !useRemoteNode && !isTurtleCoindRunning() {
		...
		log.Info("Opening CPACoind and waiting for it to be ready.")
	...
	log.Info("Opening cpa-service and waiting for it to be ready.")
	...
	for !successLaunchingWalletd && time.Duration(timesCheckLog)*timeBetweenChecks < maxWaitingTime {
		...
		for {
			...
			if strings.Contains(line, "Wallet loading is finished.") {
				...
				log.Info("cpa-service ready ('Wallet loading is finished.').")
				break
			}
			if strings.Contains(line, "Imported block with index") {
				timesCheckLog = 0
			...
			if strings.Contains(line, "INFO    Stopped") {
				...
				if len(listWalletdErrors) > 0 {
					...
					errorMessage = "cpa-service stopped with unknown error"
				}
	...
	if err != nil {
		killWalletd()
		return errors.New("error communicating with cpa-service via rpc")
	}
...
func GracefullyQuitWalletd() {
	...
	if WalletdOpenAndRunning || cmdWalletd != nil {
		...
		if isPlatformWindows {
			...
			if err != nil {
				log.Error("failed to kill cpa-service: " + err.Error())
			} else {
				log.Info("cpa-service killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdWalletd.Process.Kill(); err != nil {
					log.Warning("failed to kill cpa-service: " + err.Error())
				}
				log.Info("cpa-service killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("cpa-service finished with error: " + err.Error())
				}
				log.Info("cpa-service killed without error")
			}
		}
...
func GracefullyQuitTurtleCoind() {
	...
	if cmdTurtleCoind != nil {
		...
		if isPlatformWindows {
			...
			if err != nil {
				log.Error("failed to kill cpaCoind: " + err.Error())
			} else {
				log.Info("cpaCoind killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdTurtleCoind.Process.Kill(); err != nil {
					log.Warning("failed to kill cpaCoind: " + err.Error())
				}
				log.Info("cpaCoind killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("cpaCoind finished with error: " + err.Error())
				}
				log.Info("cpaCoind killed without error")
			}
		}
...
// CreateWallet calls cpawalled-service to create a new wallet. If privateViewKey, privateSpendKey and mnemonicSeed are empty strings, a new wallet will be generated. If they are not empty, a wallet will be generated from those keys or from the seed (import)
// walletFilename is the filename chosen by the user. The created wallet file will be located in the same folder as cpawalled-service.
...
func CreateWallet(walletFilename string, walletPassword string, walletPasswordConfirmation string, privateViewKey string, privateSpendKey string, mnemonicSeed string, scanHeight string) (err error) {
	...
	if WalletdOpenAndRunning {
		return errors.New("cpawalled-service is already running. It should be stopped before being able to generate a new wallet")
	}
