...
var (
	...
	stringBackupKeys            = ""
	rateUSDTRTL                 float64 // USD value for 1 CPA
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " CPA (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 6, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("CPAs sent successfully", 4000)
}
...
func startWalletWithWalletInfo(pathToWallet string, passwordWallet string) bool {
	...
	if err != nil {
		log.Warn("error starting cpawalled-service with provided wallet info. error: ", err)
		qmlBridge.FinishedLoadingWalletd()
		qmlBridge.DisplayErrorDialog("Error opening wallet.", err.Error())
		return false
	}

	log.Info("success starting cpawalled-service")
...
func exportListTransactions() {
	...
	for _, transfer := range transfers {
		...
		feeString := strconv.FormatFloat(transfer.Fee, 'f', 6, 64)
